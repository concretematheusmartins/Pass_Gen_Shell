...
# VERSÃO            : 1.7.2
# LICENÇA           : GPL3
# PEQUENA-DESCRICÃO : Gerar senhas via terminal shell
...
if [ -z "$max" ] || [ "$max" -eq 0 ]; then
  echo -e "${verde} Informe a QTD de caracteres da senha: ${fecha}"
  read -r max
fi

case $max in
  ''|*[!0-9]*)
    echo -e "${vermelho} Insira apenas números quanto ao TAMANHO de \
senha gerada ${fecha}"
    return 1
    ;;
  [0-9]*)
    echo -e "${verde} Informe o TIPO de senha que deseja: ${fecha}
    ${amarelo} 1 - Senha apenas números ${fecha}
    ${amarelo} 2 - Senha com LeTrAs e númer0s ${fecha}
    ${amarelo} 3 - Senha com LeTrAs, númer0s e s!mb0lo$ ${fecha}"
    read -r tipo
  case "$tipo" in
    ''|*[!0-9]*)
      echo -e "${vermelho} Insira apenas números referente ao TIPO \
da senha a ser gerada ${fecha}" >&2
      return 2
      ;;
    1)
...
case "$max" in
  -h | --help )
    echo -e "${verde} Programa para gerar passwords com complexidade
                      alfanumérica e com caracteres
                      especiais rapidamente via terminal ${fecha}"
    echo -e "${verde} Autor mateuscomh ${fecha}"
    exit 0
    ;;
  -v | --version )
